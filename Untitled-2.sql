...
SELECT S#,SNAME FROM S WHERE 'C2' IN (SELECT S# FROM SC WHERE S# = S.S#);
SELECT S#,SNAME FROM S WHERE EXIST (SELECT * FROM SC WHERE SC.S# = S.S# AND C# = 'C2');
SELECT X.S# FROM SC AS X, SC AS Y WHERE X.S# = Y.S# AND C.C# = 'C2' AND Y.C# = 'C4';
...
(SELECT S.S#,SNAME FROM S,SC WHERE S.S# = SC.S# AND C# = 'C2') UNION ALL
(SELECT S.S#,SNAME FROM S,SC WHERE S.S# = SC.S# AND C# = 'C2')

(SELECT S.S#,SNAME FROM S,SC WHERE S.S# = SC.S# AND C# = 'C2') EXCEPT
(SELECT S.S#,SNAME FROM S,SC WHERE S.S# = SC.S# AND C# = 'C2')

(SELECT S.S#,SNAME FROM S,SC WHERE S.S# = SC.S# AND C# = 'C2') INTERSECT
(SELECT S.S#,SNAME FROM S,SC WHERE S.S# = SC.S# AND C# = 'C2')
